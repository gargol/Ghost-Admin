...
const stubSuccessfulUpload = function (server, delay = 0) {
    server.post('/ghost/api/v3/members/csv/', function () {
...
describe('Integration: Component: gh-members-importer', function () {
    setupRenderingTest();
    ...
    beforeEach(function () {
        ...
        this.set('uploadUrl', '/ghost/api/v3/members/csv/');
    ...
    it('renders', async function () {
        await render(hbs`{{gh-members-importer}}`);
    ...
    it('allows file input "accept" attribute to be changed', async function () {
        ...
        await render(hbs`{{gh-members-importer accept="application/zip"}}`);
        expect(
            find('input[type="file"]').getAttribute('accept'),
    ...
    it('renders form with supplied label text', async function () {
        ...
        await render(hbs`{{gh-members-importer labelText=labelText}}`);
    ...
    it('fires fileSelected action on file selection', async function () {
        ...
        await render(hbs`{{gh-members-importer url=uploadUrl fileSelected=(action fileSelected)}}`);
        await fileUpload('input[type="file"]', ['test'], {name: 'test.csv'});
